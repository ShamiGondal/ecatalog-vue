  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(branchMain)[0] && unref(branchMain)[0].branch_waze) {
        _push(`<a class="flex items-center mt-4 w[30%]"${ssrRenderAttr("href", unref(branchMain)[0].branch_waze)}><img${ssrRenderAttr("src", _imports_3)} alt="trophy" class="w-8" style="${ssrRenderStyle({ "margin-top": "-20px" })}"></a>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
      ...
      if (unref(normalBranch)) {
        ...
        ssrRenderList(unref(normalBranch), (branch, index) => {
          ...
          if (branch.branch_waze) {
            _push(`<div class="flex items-center mt-4 w[30%]"${ssrRenderAttr("href", branch.branch_waze)}><img${ssrRenderAttr("src", _imports_3)} alt="trophy" class="w-8" style="${ssrRenderStyle({ "margin-top": "-20px" })}"></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div>`);
          if (index < unref(normalBranch).length) {
            _push(ssrRenderComponent(Line, null, null, _parent));
          } else {
            _push(`<!---->`);
          }
...
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/contact-us/[name].vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};

export { _sfc_main as default };
//# sourceMappingURL=_name_-RNaD_SJH.mjs.map
